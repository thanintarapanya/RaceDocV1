-- RaceDocV1 A4 Print Background Orientation
-- Purpose: support full-color portrait and landscape A4 background assets per Event.

alter table public.print_background_assets
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'print_background_assets_orientation_check'
      and conrelid = 'public.print_background_assets'::regclass
  ) then
    alter table public.print_background_assets
      add constraint print_background_assets_orientation_check
      check (orientation in ('portrait', 'landscape'));
  end if;
end $$;

drop index if exists print_background_assets_one_default_per_event_uk;

create unique index if not exists print_background_assets_one_default_per_event_orientation_uk
  on public.print_background_assets (event_id, orientation)
  where is_default = true;

create index if not exists print_background_assets_event_orientation_idx
  on public.print_background_assets (event_id, orientation);
...
as $$
  with organization_rows as (
    ...
    select coalesce(jsonb_agg(jsonb_build_object('printBackgroundAssetId', pba.id, 'eventId', e.id, 'eventName', e.name, 'seasonId', e.season_id, 'eventOrder', e.event_order, 'title', pba.title, 'orientation', pba.orientation, 'isDefault', pba.is_default, 'fileAssetId', fa.id, 'bucket', fa.bucket, 'path', fa.path, 'filename', fa.filename, 'mimeType', fa.mime_type, 'sizeBytes', fa.size_bytes) order by e.event_order, pba.orientation, pba.is_default desc, pba.title), '[]'::jsonb) as print_background_assets
...
drop function if exists public.save_organizer_print_background_asset(uuid, uuid, text, boolean, text, text, text, bigint);
...
declare
  ...
  v_title text;
  v_orientation text := lower(coalesce(nullif(btrim(p_orientation), ''), 'portrait'));
  v_old_values jsonb;
  v_new_values jsonb;
begin
  ...
  if v_orientation not in ('portrait', 'landscape') then
    raise exception 'Print background orientation must be portrait or landscape.';
  end if;
  ...
  if p_print_background_asset_id is not null then
    ...
    values (p_event_id, v_file_asset_id, v_title, v_orientation, coalesce(p_is_default, false))
    returning id, to_jsonb(public.print_background_assets.*) into v_print_background_asset_id, v_new_values;
  end if;
...
as $$
  with report_context as (
    ...
    order by
      case
        ...
        when br.orientation = 'portrait' and br.is_default then 1
        when br.is_default then 2
        else 3
      end,
      br.orientation,
      br.title
  ...
  select jsonb_build_object(
    ...
    'printBackgroundAssets', coalesce((
      select jsonb_agg(br.background order by br.orientation, br.is_default desc, br.title)
      from background_rows br
    ), '[]'::jsonb)
  )
...
revoke execute on function public.save_organizer_print_background_asset(uuid, uuid, text, text, boolean, text, text, text, bigint) from public, anon;
...
grant execute on function public.save_organizer_print_background_asset(uuid, uuid, text, text, boolean, text, text, text, bigint) to authenticated, service_role;
