-- RaceDocV1 Competitor Request Consent & Review Notifications
-- Purpose: complete the PRD notification flow for Team Manager racer consent
-- and dynamic reviewer assignment.
...
begin
  ...
  if new.status not in (
    'Need Racer Approval'::public.competitor_request_status,
  ...
  v_title := case new.status
    when 'Need Racer Approval'::public.competitor_request_status then 'Racer Consent Required / Queue ' || coalesce(v_context.queue_no, '--')
  ...
  v_body := case new.status
    when 'Need Racer Approval'::public.competitor_request_status then concat_ws(
      E'\n',
      ...
      'A Team Manager submitted this request on your behalf. Open it to approve or reject racer consent.'
    )
  ...
  if new.status = 'Need Racer Approval'::public.competitor_request_status then
  ...
  elsif new.status = 'Pending'::public.competitor_request_status then
    insert into public.notifications (
      recipient_profile_id,
      channel,
      title,
...
create or replace function public.create_competitor_request_review_notifications()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if new.status <> 'Pending'::public.request_approval_status then
    return new;
  end if;
  ...
  join public.competitor_requests cr on cr.id = ra.competitor_request_id
  join public.entry_forms ef on ef.id = cr.entry_form_id
  join public.events ev on ev.id = ef.event_id
  join public.series_races sr on sr.id = ef.series_race_id
  join public.grades g on g.id = ef.grade_id
  ...
  if v_context is null or v_context.deleted_at is not null then
    return new;
  end if;

  v_title := 'Competitor Request Review Assigned / Queue ' || coalesce(v_context.queue_no, '--');
  v_body := concat_ws(
    E'\n',
    coalesce(v_context.event_name, 'Event') || ' / ' || coalesce(v_context.series_class, 'Class') || ' / Car #' || coalesce(v_context.car_number, '--'),
    'Topic: ' || coalesce(v_context.topic, 'Competitor Request'),
    'Role: ' || coalesce(new.approver_role_code, 'Reviewer'),
    'Secretary/Admin assigned this request for your recommendation.'
  );
...
drop trigger if exists trg_create_competitor_request_review_notifications on public.request_approvals;
create trigger trg_create_competitor_request_review_notifications
after insert on public.request_approvals
for each row
execute function public.create_competitor_request_review_notifications();
...
revoke execute on function public.create_competitor_request_review_notifications() from public, anon, authenticated;
grant execute on function public.create_competitor_request_review_notifications() to service_role;
