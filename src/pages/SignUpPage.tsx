...
export function SignUpPage() {
  ...
  async function handleSignUp(event: FormEvent<HTMLFormElement>) {
    ...
    if (data.session) {
      setSuccessMessage('Account created. Continue to complete your profile.')
    } else {
      ...
      <section className="mx-auto grid min-h-[calc(100svh-4rem)] w-full max-w-6xl items-center gap-10 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        >
          <div className="border-l-2 border-primary pl-5">
            ...
            <h1 className="mt-4 text-4xl font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">
              Start with a verified racing identity.
            </h1>
            <p className="mt-4 text-base leading-7 text-zinc-600 dark:text-zinc-400">
              Create an account first. Your profile and permitted role are completed in the next onboarding step.
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          <form onSubmit={handleSignUp} className="space-y-4">
            ...
            <label className="block">
              <span className="text-sm font-medium text-zinc-800 dark:text-zinc-200">Confirm Password</span>
              <input
                type="password"
            ...
            >
              ...
              {pending ? <UserPlus size={18} aria-hidden="true" /> : <ArrowRight size={18} aria-hidden="true" />}
            </motion.button>
          </form>
